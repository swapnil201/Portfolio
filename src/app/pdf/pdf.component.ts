...
export class PdfComponent implements OnInit {
  ...
  download() {
    ...
    if (
      this.certificationItems !== undefined &&
      this.certificationItems !== [] &&
      this.certificationItems.length > 0
    ) {
      this.switchPage(doc);
      this.addCertificatesPageContent(doc);
    }
    if (
      this.publicationItems !== undefined &&
      this.publicationItems !== [] &&
      this.publicationItems.length > 0
    ) {
      this.switchPage(doc);
      this.addPublicationsPageContent(doc);
    }
    if (
      this.talkItems !== undefined &&
      this.talkItems !== [] &&
      this.talkItems.length > 0
    ) {
      this.switchPage(doc);
    ...
    if (
      this.volunteerItems !== undefined &&
      this.volunteerItems !== [] &&
      this.volunteerItems.length > 0
    ) {
      this.switchPage(doc);
  ...
  private addGeneralDescription(doc) {
    ...
    const splittedDescription = doc.splitTextToSize(
      this.generalData.description,
      this.maximumHorizontalLength
    );
  ...
  private addInterests(doc) {
    if (
      this.interestItems !== undefined &&
      this.interestItems !== [] &&
      this.interestItems.length > 0
    ) {
      doc.setFontSize(14);
      doc.setFontType('bold');
      ...
      const splittedInterests = doc.splitTextToSize(
        interests,
        this.maximumHorizontalLength
      );
  ...
  private addCvItem(doc, item: CvItem) {
    ...
    if (item.description !== undefined && item.description !== '') {
      ...
      const splittedDescription = doc.splitTextToSize(
        item.description,
        this.maximumHorizontalLength - 10
      );
    ...
    if (item.tags !== undefined && item.tags.length > 0) {
      ...
      const splittedTags = doc.splitTextToSize(
        tags,
        this.maximumHorizontalLength - 10
      );
